...
public class FederationClusterDemo {

    private static CountDownLatch latch = new CountDownLatch(1);

    private static final String QUEUE = "fed.queue";
    private static final String EXCHANGE = "fed.exchange";
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        /**
         * 172.17.0.3 上游 172.17.0.4 下游 两个实例可以向任意一个发送消息,其他监听都可以收到,证明集群成功
         */
        switch (type){
            case "queue":
                System.out.println("federation 集群 queue (首次启动有时无法接收消息,第二次启动后正常?!)");
                USER = "admin";
                PASSWD = "admin";
                ExecutorService service1 = Executors.newFixedThreadPool(2);
                service1.execute(new ClusterReceiver("172.17.0.4:5672"));
                latch.await();
                service1.execute(new ClusterSender("172.17.0.3:5672"));
                service1.shutdown();
                break;
            case "exchange":
                System.out.println("federation 集群 exchange (首次启动有时无法接收消息,第二次启动后正常?!)");
                USER = "admin";
                PASSWD = "admin";
                ExecutorService service2 = Executors.newFixedThreadPool(2);
                service2.execute(new ClusterReceiver("172.17.0.4:5672",EXCHANGE));
                latch.await();
                service2.execute(new ClusterSender("172.17.0.3:5672",EXCHANGE));
                service2.shutdown();
                break;
        }
    ...
    static class ClusterSender implements Runnable{
        ...
        public void run() {
            ...
            try {
                ...
                System.out.println(connection.getAddress().getHostAddress()+ " " + factory.getPort()+" 发送消息完毕 "+ MSG_NUM);
                channel.close();
                connection.close();
            } catch (IOException e) {
                e.printStackTrace();
    ...
    static class ClusterReceiver implements Runnable{
        ...
        public void run() {
            ...
            try {
                ...
                System.out.println(connection.getAddress().getHostAddress() +" downstream 客户端启动,等待消息...");
                latch.countDown();
            } catch (IOException e) {
                e.printStackTrace();
            } catch (TimeoutException e) {
